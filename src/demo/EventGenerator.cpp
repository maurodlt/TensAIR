...
BasicVertex<>(tag, rank, worldSize, windowSize, comm){
    ...
    this->conceptDrifts = {
        ...
        {make_pair(3,7)},  // First pair
        {make_pair(2,8)},  // Second pair
        {make_pair(5,1)},  // Third pair
        {make_pair(0,9)},  // Fourth pair
        {make_pair(4,2)}  // Fifth pair
    };
}
...
int EventGenerator::addToBatch(Mini_Batch_Generator &micro_batch, std::ifstream &infile, int position){
    ...
    if (position + 1 <= micro_batch.mini_batch_size){
        ...
        int drift_id = floor(epoch / drift_frequency);

        if(drift_id >= conceptDrifts.size()){ 
            int firstNumber = distrib(gen);
            int secondNumber;
            do {
                secondNumber = distrib(gen);
            } while (secondNumber == firstNumber);
            conceptDrifts.push_back(std::make_pair(firstNumber, secondNumber));
        }
        
        if(label_int == conceptDrifts[drift_id].first){
            label_int = conceptDrifts[drift_id].second;
        }else if(label_int == conceptDrifts[drift_id].second){
            label_int = conceptDrifts[drift_id].first;
        }
